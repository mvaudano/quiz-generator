(function ($) {
  ...
  var buildQuiz = function (input) {
   ...
   $(".quiz-container").html("<div class='progress'>Question " + qnumber + "&nbsp;sur&nbsp;" + input.length + "</div><div class='qq-question'>" + ((input[currentQuestion].description !== "") ? ("<div class='qq-description'>"+input[currentQuestion].description+"</div>") : "")+ "<div class='quiz-question'>" + input[currentQuestion].question + "</div></div>" +
  ...
  var checkAnswer = function () {
    if ($(".selected").length > 0) {
      ...
      if (reponse == input[currentQuestion].bonnereponse) {
        ...
        score++;
        displayProgress();
        $(".reponse").html("<p>Bonne r&eacute;ponse !</p><p>" + input[currentQuestion].correct + "</p>");

      } else {
        trackEvent(
          'q' + qnumber + '-reponseed-incorrectly',
          'Q' + qnumber + ' reponseed incorrectly');
        $(".reponse").html("<p>D&eacute;sol&eacute;, mauvaise réponse !</p><p>La bonne r&eacute;ponse &eacute;tait : <span class=\"bonnereponse\">" + input[currentQuestion].bonnereponse + "</span>.</p><p>" + input[currentQuestion].incorrect + "</p>");
      }
      if (currentQuestion != (input.length-1)) {
        $(".reponse").append("<button class='qq-button btn next'>Suivant</button>");
        $('.next').on('click', nextQuestion);
      } else {
        $(".reponse").append("<button class='qq-button check-score'>Voir le score final</button>");
  ...
  var finalScore = function () {
    trackEvent(
      ...
      'Scored ' + score + ' of ' + input.length);
    trackEvent('completed', 'Quiz termin&eacute;');
    switch (pub) {
      case 'vox':
        account = voxdotcom;
        break;
    ...
    $(".quiz-container")
      .html("<div class='scorecard'><p>Vous avez correctement r&eacute;pondu &agrave;</p><p class=\"tt3_capital\">" + score + "&nbsp;r&eacute;ponses sur&nbsp;" + input.length + "</p><p class=\"defiez\">D&eacute;fiez vos amis !</p><div id='social-media'><ul><li><a class=\"fb-share\" href='http://www.facebook.com/sharer.php?u=" + link + "' target='_blank'>" + facebook + "</a></li><li><a class=\"twitter-share\" href='http://twitter.com/home?status=J%27ai obtenu le score de " + score + "/" + input.length + " sur ce quiz" + link + " via @" + account + "' target='_blank'>" + twitter   + "</a></li><li><a class=\"gplus-share\" href='https://plus.google.com/share?url=" + link + "' target='_blank'>" + google + "</a></li></ul></div></div>");
  ...
  $('head').append('<meta charset="utf-8"><link rel="stylesheet" href="stylesheets/quiz.css" type="text/css" />');
  ...
  $(document).ready(function () {
    trackEvent('loaded', 'Quiz charg&eacute;');
    unpackQuizHack();
  });
})(jQuery);
